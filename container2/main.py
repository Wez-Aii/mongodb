...
USER_DM = "user_dm"
FACTORY_DM = "factory_dm"
FACTORY_ACCOUNT_DM = "factory_account_dm"
MACHINE_DM = "machine_dm"
...
TOKEN_BALANCE_ACCOUNT_DM = "token_balance_account_dm"
TOKEN_USAGE_ACCOUNT_DM = "token_usage_account_dm"
TOKEN_PURCHASE_ACCOUNT_DM = "token_purchase_account_dm"
TOKEN_GIFT_ACCOUNT_DM = "token_gift_account_dm"
TOKEN_ADJUSTMENT_ACCOUNT_DM = "token_adjustment_account_dm"
...
BEFORE_INSERT_VALIDATE_TOKEN_PURCHASE_TXN = "before_insert_validate_token_purchase_txn"
INSERT_TOKEN_TXN_JOURNAL = "insert_token_txn_journal"
INSERT_OR_UPDATE_ACCOUNTS = "insert_or_update_accounts"
...
TOKEN_PURCHASE_TXN_BEFORE_INSERTED_TRIGGER = "token_purchase_txn_before_inserted_trigger"
...
DEBUGING_TABLE = "debuging_table"

DATABASE_TABLES = {
    DEBUGING_TABLE : f"""
        CREATE TABLE {DEBUGING_TABLE} (
            id SERIAL PRIMARY KEY,
            debug_name VARCHAR(255),
    ...
    """,
    FACTORY_DM : f"""
        CREATE TABLE {FACTORY_DM} (
            id SERIAL PRIMARY KEY,
            factory_name VARCHAR(255),
            factory_address VARCHAR(255),
            factory_location VARCHAR(255),
            created_by INTEGER REFERENCES {USER_DM}(id),
    ...
    FACTORY_ACCOUNT_DM: f"""
        CREATE TABLE {FACTORY_ACCOUNT_DM} (
            id SERIAL PRIMARY KEY,
            factory_id INTEGER REFERENCES {FACTORY_DM}(id),
            account_name VARCHAR(255),
            timestamp TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
        );
    """,
    MACHINE_DM : f"""
        CREATE TABLE {MACHINE_DM} (
            id SERIAL PRIMARY KEY,
    ...
    PROMO_RECORD_DM: f"""
        CREATE TABLE {PROMO_RECORD_DM} (
            id SERIAL PRIMARY KEY,
            update_by INTEGER REFERENCES {USER_DM}(id),
            approved_by INTEGER REFERENCES {USER_DM}(id),
    ...
    BATCH_RECORD_DM: f"""
        CREATE TABLE {BATCH_RECORD_DM} (
            id SERIAL PRIMARY KEY,
            factory_id INTEGER REFERENCES {FACTORY_DM}(id),
            machine_id INTEGER REFERENCES {MACHINE_DM}(id),
    ...
    TOKEN_PURCHASE_TXN_DM: f"""
        CREATE TABLE {TOKEN_PURCHASE_TXN_DM} (
            ...
            token_to_bath_exchange_rate_id INTEGER NOT NULL REFERENCES {TOKEN_EXCHANGE_RATE_DM}(id),
    ...
    TOKEN_LONGAN_LOT_USAGE_TXN_DM: f"""
        CREATE TABLE {TOKEN_LONGAN_LOT_USAGE_TXN_DM} (
            ...
            token_to_longan_rate DECIMAL(6,3),
    ...
    """,
    TOKEN_BALANCE_ACCOUNT_DM: f"""
        CREATE TABLE {TOKEN_BALANCE_ACCOUNT_DM} (
            id SERIAL PRIMARY KEY,
            factory_id INTEGER REFERENCES {FACTORY_DM}(id),
    ...
    """,
    TOKEN_USAGE_ACCOUNT_DM: f"""
        CREATE TABLE {TOKEN_USAGE_ACCOUNT_DM} (
            id SERIAL PRIMARY KEY,
            factory_id INTEGER REFERENCES {FACTORY_DM}(id),
            machine_id INTEGER REFERENCES {MACHINE_DM}(id),
            last_token_journal_id INTEGER REFERENCES {TOKEN_TXN_JOURNAL_DM}(id),    
    ...
    """,
    TOKEN_PURCHASE_ACCOUNT_DM: f"""
        CREATE TABLE {TOKEN_PURCHASE_ACCOUNT_DM} (
            id SERIAL PRIMARY KEY,
            factory_id INTEGER REFERENCES {FACTORY_DM}(id),
            last_token_journal_id INTEGER REFERENCES {TOKEN_TXN_JOURNAL_DM}(id),    
            account_id INTEGER REFERENCES {FACTORY_ACCOUNT_DM}(id),
    ...
    """,
    TOKEN_GIFT_ACCOUNT_DM: f"""
        CREATE TABLE {TOKEN_GIFT_ACCOUNT_DM} (
            id SERIAL PRIMARY KEY,
            factory_id INTEGER REFERENCES {FACTORY_DM}(id),
            last_token_journal_id INTEGER REFERENCES {TOKEN_TXN_JOURNAL_DM}(id),    
            account_id INTEGER REFERENCES {FACTORY_ACCOUNT_DM}(id),
    ...
    """,
    TOKEN_ADJUSTMENT_ACCOUNT_DM: f"""
        CREATE TABLE {TOKEN_ADJUSTMENT_ACCOUNT_DM} (
            id SERIAL PRIMARY KEY,
            factory_id INTEGER REFERENCES {FACTORY_DM}(id),
            last_token_journal_id INTEGER REFERENCES {TOKEN_TXN_JOURNAL_DM}(id),    
            account_id INTEGER REFERENCES {FACTORY_ACCOUNT_DM}(id),
...
PROCEDURES_CREATE_SQL_COMMANDS_DICT = {
    ...
    INSERT_TOKEN_LONGAN_LOT_USAGE_TXN : F"""
        ...
        BEGIN
            ...
            FROM {TOKEN_LONGAN_RATE_DM} WHERE NEW.date >= start_apply_date AND (end_date IS NULL OR NEW.date < end_date);
            IF _token_longan_rate_id IS NOT NULL AND _token_to_longan_rate IS NOT NULL THEN
                ...
                INSERT INTO {TOKEN_LONGAN_LOT_USAGE_TXN_DM} (
                    ...
                    token_to_longan_rate, token_usage_amt, date
                    ) VALUES (
                    NEW.id, _token_longan_rate_id, NEW.factory_id,
                    NEW.machine_id, NEW.lot_total_longan,
                    _token_to_longan_rate, _token_usage_amt, NEW.date
                    );
    ...
    UPDATE_LATEST_TOKEN_EXCHANGE_RATE_END_DATE : f"""
        ...
        RETURNS TRIGGER AS $$
        BEGIN
            IF EXISTS (SELECT 1 FROM {TOKEN_EXCHANGE_RATE_DM} WHERE start_apply_date > NEW.start_apply_date ORDER BY start_apply_date ASC LIMIT 1) THEN
                NEW.end_date := (SELECT start_apply_date FROM {TOKEN_EXCHANGE_RATE_DM} WHERE start_apply_date > NEW.start_apply_date ORDER BY start_apply_date ASC LIMIT 1);
            ELSE
    ...
    UPDATE_LATEST_TOKEN_TO_LONGAN_RATE_END_DATE : f"""
        ...
        RETURNS TRIGGER AS $$
        BEGIN
            IF EXISTS (SELECT 1 FROM {TOKEN_LONGAN_RATE_DM} WHERE start_apply_date > NEW.start_apply_date ORDER BY start_apply_date ASC LIMIT 1) THEN
                NEW.end_date := (SELECT start_apply_date FROM {TOKEN_LONGAN_RATE_DM} WHERE start_apply_date > NEW.start_apply_date ORDER BY start_apply_date ASC LIMIT 1);
            ELSE
        ...
        $$ LANGUAGE plpgsql;
    """,    
    BEFORE_INSERT_VALIDATE_TOKEN_PURCHASE_TXN : f"""
        CREATE OR REPLACE FUNCTIOn {BEFORE_INSERT_VALIDATE_TOKEN_PURCHASE_TXN}()
        RETURNS TRIGGER AS $$
        DECLARE
            exchange_rate DECIMAL;
            exchange_rate_calculated_purchase_token_amt DECIMAL;
            _promo_token_amt DECIMAL;
            calculated_promo_token_amt DECIMAL;
        BEGIN
            SELECT token_to_bath_exchange_rate INTO exchange_rate FROM {TOKEN_EXCHANGE_RATE_DM} 
            WHERE id = NEW.token_to_bath_exchange_rate_id AND NEW.date >= start_apply_date AND (end_date IS NULL OR NEW.date < end_date);
            exchange_rate_calculated_purchase_token_amt := exchange_rate * NEW.bath_amt;
            -- INSERT INTO {DEBUGING_TABLE} (debug_name, decimal_type) VALUES ('exchange_rate_calculated_purchase_token_amt', exchange_rate_calculated_purchase_token_amt);
            IF NEW.promo_token_amt IS NULL THEN
                _promo_token_amt := 0.0;
            END IF;
            -- promo token amt will be calculated using promo id provided and the bath amt spend
            -- for now checking promo function will not available yet
            calculated_promo_token_amt := _promo_token_amt;
            -- INSERT INTO {DEBUGING_TABLE} (debug_name, decimal_type) VALUES ('calculated_promo_token_amt', calculated_promo_token_amt);
            IF (exchange_rate_calculated_purchase_token_amt = NEW.purchase_token_amt) AND (calculated_promo_token_amt = _promo_token_amt) THEN
                INSERT INTO {DEBUGING_TABLE} (debug_name, bool_type) VALUES ('both match the calculation', true);
                IF NEW.total_token_amt = (NEW.purchase_token_amt + _promo_token_amt) THEN
                    -- INSERT INTO {DEBUGING_TABLE} (debug_name, bool_type) VALUES ('both add up to total amt', true);
                    RETURN NEW;
                END IF;
            END IF;
            RETURN NULL;
    ...
    INSERT_TOKEN_TXN_JOURNAL : f"""
        ...
        BEGIN
            ...
            IF _account_id IS NOT NULL THEN
                ...
                txn_id, txn_token_amt, txn_entry_type, date
                ) VALUES 
                (
                    NEW.factory_id, _machine_id, _account_id, _txn_type_id,
                    NEW.id, _txn_token_amt, _first_txn_entry_type, NEW.date
                ),
                (
                    NEW.factory_id, _machine_id, _wallet_account_id, _txn_type_id,
                    NEW.id, _txn_token_amt, _second_txn_entry_type, NEW.date
                );
            END IF;
            RETURN NEW;
        END;
        $$ LANGUAGE plpgsql;
    """,
    INSERT_OR_UPDATE_ACCOUNTS : f"""
        CREATE OR REPLACE FUNCTION {INSERT_OR_UPDATE_ACCOUNTS}()
        RETURNS TRIGGER AS $$
        DECLARE
        ...
        BEGIN
            ...
            IF _account_name = 'wallet account' THEN
                ...
                END IF;
                IF EXISTS (SELECT 1  FROM {TOKEN_BALANCE_ACCOUNT_DM} WHERE date = NEW.date AND factory_id = NEW.factory_id) THEN
                    UPDATE {TOKEN_BALANCE_ACCOUNT_DM} 
                    SET last_token_journal_id = NEW.id, 
                    token_balance_amt = token_balance_amt + NEW.txn_token_amt, 
                    timestamp = CURRENT_TIMESTAMP 
                    WHERE date = NEW.date AND factory_id = NEW.factory_id;
                ELSE
                    SELECT token_balance_amt INTO _latest_token_balance_amt FROM {TOKEN_BALANCE_ACCOUNT_DM}
                    WHERE factory_id = NEW.factory_id AND account_id = NEW.account_id AND date < NEW.date ORDER BY date DESC, timestamp DESC LIMIT 1;
                    IF _latest_token_balance_amt IS NULL THEN
                        _latest_token_balance_amt := 0;
                    END IF;
                    INSERT INTO {TOKEN_BALANCE_ACCOUNT_DM}(
                        factory_id, last_token_journal_id, account_id, token_balance_amt, date
                        ) VALUES (
                            NEW.factory_id, NEW.id, NEW.account_id,_latest_token_balance_amt + NEW.txn_token_amt, NEW.date 
                            );
                END IF;
                -- adjust the account balance according to the new purchase txn insert data
                UPDATE {TOKEN_BALANCE_ACCOUNT_DM} SET token_balance_amt = token_balance_amt + NEW.txn_token_amt, timestamp = CURRENT_TIMESTAMP WHERE date > NEW.date AND account_id = NEW.account_id;
            ELSIF _account_name = 'purchase account' THEN
                IF EXISTS (SELECT 1  FROM {TOKEN_PURCHASE_ACCOUNT_DM} WHERE date = NEW.date AND factory_id = NEW.factory_id) THEN
                    UPDATE {TOKEN_PURCHASE_ACCOUNT_DM} 
                    SET last_token_journal_id = NEW.id, 
                    token_purchase_amt = token_purchase_amt + NEW.txn_token_amt, 
                    timestamp = CURRENT_TIMESTAMP 
                    WHERE date = NEW.date AND factory_id = NEW.factory_id;
                ELSE
                    INSERT INTO {TOKEN_PURCHASE_ACCOUNT_DM}(
                        factory_id, last_token_journal_id, account_id, token_purchase_amt, date
                        ) VALUES (
                            NEW.factory_id, NEW.id, NEW.account_id, NEW.txn_token_amt, NEW.date 
                            );
                END IF;
            ELSIF _account_name = 'usage account' THEN
                IF EXISTS (SELECT 1  FROM {TOKEN_USAGE_ACCOUNT_DM} WHERE date = NEW.date AND factory_id = NEW.factory_id AND machine_id = NEW.machine_id) THEN
                    UPDATE {TOKEN_USAGE_ACCOUNT_DM} 
                    SET last_token_journal_id = NEW.id, 
                    token_usage_amt = token_usage_amt + NEW.txn_token_amt, 
                    timestamp = CURRENT_TIMESTAMP 
                    WHERE date = NEW.date AND factory_id = NEW.factory_id;
                ELSE
                    INSERT INTO {TOKEN_USAGE_ACCOUNT_DM}(
                        factory_id, machine_id, last_token_journal_id, account_id, token_usage_amt, date
                        ) VALUES (
                            NEW.factory_id, NEW.machine_id, NEW.id, NEW.account_id, NEW.txn_token_amt, NEW.date 
                            );
                END IF;            
            ELSIF _account_name = 'gift account' THEN
                IF EXISTS (SELECT 1  FROM {TOKEN_GIFT_ACCOUNT_DM} WHERE date = NEW.date AND factory_id = NEW.factory_id) THEN
                    UPDATE {TOKEN_GIFT_ACCOUNT_DM} 
                    SET last_token_journal_id = NEW.id, 
                    token_gift_amt = token_gift_amt + NEW.txn_token_amt, 
                    timestamp = CURRENT_TIMESTAMP 
                    WHERE date = NEW.date AND factory_id = NEW.factory_id;
                ELSE
                    INSERT INTO {TOKEN_GIFT_ACCOUNT_DM}(
            ...
            ELSIF _account_name = 'adjustment account' THEN
                IF NEW.txn_entry_type = 'debit' THEN
                    NEW.txn_token_amt := -NEW.txn_token_amt;
                END IF;
                IF EXISTS (SELECT 1  FROM {TOKEN_ADJUSTMENT_ACCOUNT_DM} WHERE date = NEW.date AND factory_id = NEW.factory_id) THEN
                    UPDATE {TOKEN_ADJUSTMENT_ACCOUNT_DM} 
                    SET last_token_journal_id = NEW.id, 
                    token_adjust_amt = token_adjust_amt + NEW.txn_token_amt, 
                    timestamp = CURRENT_TIMESTAMP 
                    WHERE date = NEW.date AND factory_id = NEW.factory_id;
                ELSE
                    INSERT INTO {TOKEN_ADJUSTMENT_ACCOUNT_DM}(
                        factory_id, last_token_journal_id, account_id, token_adjust_amt, date
                        ) VALUES (
                            NEW.factory_id, NEW.id, NEW.account_id, NEW.txn_token_amt, NEW.date 
                            );
                END IF;
...
TRIGGERS_CREATE_SQL_COMMAND_STRING = f"""
        ...
        -- Trigger on new token purchase txn data before inserted to validate token_purchase_txn data
        DO $$
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = '{TOKEN_PURCHASE_TXN_BEFORE_INSERTED_TRIGGER}') THEN
                CREATE TRIGGER {TOKEN_PURCHASE_TXN_BEFORE_INSERTED_TRIGGER}
                BEFORE INSERT ON {TOKEN_PURCHASE_TXN_DM}
                FOR EACH ROW
                EXECUTE FUNCTION {BEFORE_INSERT_VALIDATE_TOKEN_PURCHASE_TXN}();
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = '{TOKEN_TXN_JOURNAL_INSERTED_TRIGGER}') THEN
                ...
                FOR EACH ROW
                EXECUTE FUNCTION {INSERT_OR_UPDATE_ACCOUNTS}();
            END IF;
        END $$;
    """
